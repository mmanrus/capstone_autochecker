...
export const resubmit = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (oldSubmission?.fileData) {
      try {
        await unlinkAsync(oldSubmission.fileData);
        console.log(`Deleted old file: ${oldSubmission.fileData}`);
      } catch (unlinkError) {
        console.error(`Failed to delete old file: ${oldSubmission.fileData}`, unlinkError);
      }
    }
    ...
    try {
      ...
      console.log(`Deleted new file after failure: ${newFilePath}`);
    } catch (unlinkError) {
