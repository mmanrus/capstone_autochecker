...
export const submit = async (req: Request, res: Response) => {
  ...
  try {
    ...
    try {
      ...
      console.error(
        `Failed to delete file: ${submissionFilePath}`,
        unlinkError
      );
    }
...
export const resubmit = async (req: Request, res: Response) => {
  ...
  try {
    ...
    /**
    const oldSubmission = await submissionServices.getSubmissionById(
      submissionId
    );
    console.log("Old submission filename: ", oldSubmission?.fileName);
    ...
    if (oldSubmission?.fileName) {
      try {
        ...
        console.error(
          `Failed to delete old file: ${oldSubmission.fileName}`,
          unlinkError
        );
      }
    } */

    // 3. Resubmit with new file
    const updatedSubmission = await submissionServices.resubmit(
      subjectId,
      activityId,
      submissionId,
      newFilePath
    ...
    // Cleanup: delete the new file if DB update failed
    /**try {
      await submissionServices.setFileName(submissionId)
      await unlinkAsync(newFilePath);
      console.log(`Deleted existing file after failure: ${newFilePath}`);
    } catch (unlinkError) {
      console.error(`Failed to delete new file: ${newFilePath}`, unlinkError);
    }
 */
    res.status(500).json({
