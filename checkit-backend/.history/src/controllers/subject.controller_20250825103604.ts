...
export const joinSubjectClass = async (req: Request, res: Response) => {
  ...
  try {
    const subject = await subjectServices.joinSubjectClass(
      code,
      parseInt(userId!)
    );
    if (!subject) {
    ...
    res.status(200).json({
      message: "Successfully Joined Subject Class.",
      subject,
    })
  }  catch (error: any) {
    console.log("Error joining Class Subject: ", error)
    res.status(500).json({
      error: "An internal server error has occured.",
    });
  }
