 ...
 */
export const runCheck50 = async (activity, filePath) => {
  ...
  try {
    ...
    check50Output = stdout;

    // 4. Correctly parse the JSON string into an object.
    jsonOutput = JSON.parse(check50Output);
    ...
    return {
      ...
      rawOutput: check50Output || error.message,
      results: null,
    };
  }
