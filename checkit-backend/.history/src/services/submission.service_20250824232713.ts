...
export const getAllSubmissionBySubjectId = async (subjectId: number) => {
  const subjectSubmissions = await prisma.submission.findMany({
    where: { subjectActivity: { subjectId } },
    select: {
      id: true,
      grade: true,
      feedback: true,
...
export const getAllStudentSubmission = async (userId: number) => {
  const subjectSubmissions = await prisma.submission.findMany({
